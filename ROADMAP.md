...
- [ ] SSE transport decision for remote-client compatibility: clients that expose "custom MCP connector" UIs (notably ChatGPT) require SSE endpoints reachable from their infra, which Streamable-HTTP-on-localhost cannot satisfy. Decide and document one of two paths: (a) add an SSE transport behind the same auth/bind rules as the HTTP transport, with a documented tunneling recipe (cloudflared/ngrok) and explicit warning about exposing a personal API key over the public internet; or (b) declare ChatGPT remote-connector use out-of-scope until the vNext hosted relay, and document the decision in `docs/` so users stop filing it as a bug. Record the choice in `CHANGELOG.md`.
