...
- **Model-compatible tool profiles:** `ICUVISOR_TOOLSET=compact` exposes a reduced read-focused catalog for smaller/local models, while the default `core` and expert `full` profiles remain available for capable clients.
