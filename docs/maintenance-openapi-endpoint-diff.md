...
icuvisor keeps a pinned intervals.icu OpenAPI path and schema-name baseline in `scripts/openapidiff/baseline/intervals-openapi.json`. Maintainers use it to spot newly documented or removed upstream endpoint paths and schema names, then decide whether the change deserves a Taskplane/backlog item.
...
1. Read the generated Markdown report. It compares OpenAPI `paths` keys and `components.schemas` names. It is a human triage aid, not product approval or a product-scope decision.
2. For each added path or schema name, inspect the public intervals.icu API docs and decide whether it fits the PRD/roadmap.
