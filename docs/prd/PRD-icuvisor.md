...
5. **Coach-mode credential delegation is safe** when the coach-scoped API key is held only by the local binary and never passed as a tool parameter. _(Threat-model review before coach-mode ships.)_
