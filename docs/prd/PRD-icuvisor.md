...
- `apply_training_plan` — instantiate a workout-library folder onto the calendar from a start date. Dry-runs report existing same-day events and duplicate plan dates before writes; non-dry runs skip exact duplicate existing plan events and duplicate same-day planned entries instead of creating another copy.
