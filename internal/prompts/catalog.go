...
func CoachRosterTriagePrompt() Prompt {
	return Prompt{
		...
		Arguments: []Argument{
			{Name: "athlete_id", Title: "Athlete ID", Description: "Required intervals.icu athlete selector string; accepts 12345 or i12345. This is not an API key or credential.", Required: true},
...
func coachRosterTriageHandler(ctx context.Context, req Request) (Result, error) {
	...
	if athleteID == "" {
		return Result{}, NewUserError("missing athlete_id; provide 12345 or i12345", nil)
	}
	normalized, err := config.NormalizeAthleteID(athleteID)
	if err != nil {
		return Result{}, NewUserError("invalid athlete_id; use 12345 or i12345", err)
	}
