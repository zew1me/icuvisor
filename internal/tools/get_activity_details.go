...
const (
	...
	getActivityDetailsDescription       = "Get one activity's terse metadata and metrics by activity_id, including upstream activity tags when returned, calories_burned as active/exercise calories (distinct from wellness kcal_consumed), carbs_ingested_g for athlete-logged carb intake, carbs_used_g for upstream carbs-burned estimate, and custom_fields for athlete-defined activity custom fields when upstream provides them. If the user described an activity by date/name instead of ID, resolve it with get_activities over the athlete-local date window first. Use include_full only when raw upstream fields are needed; Strava-blocked activities return an unavailable marker instead of sparse N/A rows."
	getActivityIntervalsDescription     = "Get analyzed intervals for one activity by activity_id, including scalar custom interval fields such as lactate under custom_fields when upstream includes them. For reps/laps in a described or date-based activity, resolve the activity with get_activities first and pass the returned activity_id. Interval units are normalized to the canonical intervals.icu unit enum and raw interval payloads require include_full."
